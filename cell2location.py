...
# cell2location utils
#? you can this section on google colab (zhen xiang)
...
parser.add_argument("-gpu", action="store_true", required=False, default=False)
args = parser.parse_args()


adata_spatial = sc.read(args.sp)
#adata_spatial.__dict__['_raw'].__dict__['_var'] = adata_spatial.__dict__['_raw'].__dict__['_var'].rename(columns={'_index': 'features'})
...
adata_ref.obs[label_key] = adata_ref.obs[label_key].str.replace("/", "_")
...
"""
adata_ref.obs[label_key] = adata_ref.obs[scType]
adata_ref.obs[bcType] = adata_ref.obs[bcType].astype("category")
celltype_list = adata_ref.obs[bcType].cat.categories
adata_ref.obs[label_key] = adata_ref.obs[scType]
for i in celltype_list:
    if i != label_key:
        adata_ref.obs.loc[adata_ref.obs[bcType]==i, label_key] = i
"""
...
# Save anndata object with results
adata_file = f"./sp.denovo.expected_per_cell_type.h5ad"
